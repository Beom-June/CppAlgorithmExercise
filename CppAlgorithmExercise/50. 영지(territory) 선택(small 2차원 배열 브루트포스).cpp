...
전체 땅은 사각형의 모양의 격자로 되어 있으며, 그 사각형 땅 안에는 많은 오렌지 나무가 심겨져 있다.현수는 오렌지를 무척 좋아하여 오렌지 나무가 가장 많이 포함되는 지역을 선택하고 싶어 한다.
...
int main()
{
	int h, w, n, m;					// h, w : 길이 & n, m : 영지 크기
	int i, j, k, s;
	int sum = 0, max = -214700000;
	...
	cin >> n >> m;
	// 5행 5열까지만 돌아야 하므로, h - n + 1, w - m + 1을 넣음
	for (i = 1; i <= h - n + 1; i++)
	{
		for (j = 1; j <= w - m + 1; j++)
		{
